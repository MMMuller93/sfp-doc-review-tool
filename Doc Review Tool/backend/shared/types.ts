// ============================================================================
// Doc Review Tool v2 — Shared Types
// Used by both frontend and backend via relative imports
// ============================================================================
...
export interface PreflightResult {
  ...
  rationale: string;
  complexity?: 'simple' | 'moderate' | 'complex'; // v2: helps route to correct model
}
...
export interface Issue {
  ...
  verificationStatus?: QuoteVerificationStatus; // v2: from quote verification
}
...
export interface AnalysisMetadata {
  ...
  modelUsed: string;
  pipelineVersion?: 'v1' | 'v2'; // v2: track which pipeline produced this
  stageTimings?: Record<string, number>; // v2: per-stage timing in ms
  totalTimeMs?: number;
}
...
export interface DocumentSection {
  id: string;
  title: string;
  level: number; // 1 = Article, 2 = Section, 3 = Subsection
  pageStart: number;
  pageEnd: number;
  content: string;
}
...
export interface CrossReference {
  from: string; // Section that references
  to: string; // Section being referenced
  context: string;
}
...
export interface LLMCallConfig {
  ...
  maxTokens: number;
  systemPrompt?: string;
  cacheControl?: boolean; // Enable prompt caching (Anthropic)
}
